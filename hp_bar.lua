-- hp_bar.lua -- VENDORED, verbatim, from the engine's
-- src/battle/gen2/HpBar.lua (7908 bytes).  It was moved into the mod for ONE
-- reason: the engine's cross-generation require gate refuses every
-- require("src.*.gen2.*") on a Gen 1 boot (src/mods/Loader.lua's
-- crossGenerationDenial), so the Gen 1 arm of native.lua cannot reach the
-- module by its engine name even though the code is pure maths with no
-- requires of its own.  This file is byte-for-byte the engine's module, so a
-- Gen 1 boot computes the bar with exactly the same pixels/palette math; the
-- Gen 2 arm still requires the real engine module and is unchanged.  If the
-- engine's HpBar.lua ever changes, re-copy it here.
--
